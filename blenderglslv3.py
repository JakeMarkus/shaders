...
def build_object_shader(vertex_source, fragment_source):
    info = gpu.types.GPUShaderCreateInfo()
    ...
    vertex_source   = strip_uniform_declarations(vertex_source,   custom_names)
    fragment_source = strip_uniform_declarations(fragment_source, custom_names)
...
def compile_shader_folder(shader_folder):
    ...
    if not os.path.isfile(vertex_path):
        return None, f"Missing file: {vertex_path}"
    if not os.path.isfile(fragment_path):
        return None, f"Missing file: {fragment_path}"
    try:
        shader = build_object_shader(
            load_text_file(vertex_path),
            load_text_file(fragment_path),
        )
        return shader, ""
...
def rebuild_shader_cache(root_folder):
    ...
    for entry in sorted(os.listdir(root_folder)):
        ...
        if not os.path.isdir(folder):
            continue
        shader, error = compile_shader_folder(folder)
        _shader_cache[os.path.normpath(folder)] = {"shader": shader, "error": error}
...
def get_object_shader(shader_folder):
    ...
    shader, error = compile_shader_folder(shader_folder)
    _shader_cache[shader_folder] = {"shader": shader, "error": error}
    return shader
...
class SIMPLESHADER_OT_refresh(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, f"Refreshed. {error_count} compile error(s).")
        return {'FINISHED'}
